...
call uv run python -c "from release_tool.pre_signer import PreSigner,PreSignConfig; from pathlib import Path; PreSigner(PreSignConfig(True, r'//XIDA-SERVER/SigningExecutables/', r'//XIDA-SERVER/SigningExecutables/signed', 'XIDA GmbH')).process(Path(r'%~1'))"
if errorlevel 1 (
